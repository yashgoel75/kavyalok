...
export async function GET(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await register();
    const comp = await superCompetition.findById(id).lean();
    if (!comp) {
...
export async function PUT(
  ...
  try {
    ...
    if (isAdmin && updates) {
      const competition = await superCompetition.findById(id);
      if (!competition) {
        return NextResponse.json(
          { success: false, error: "Competition not found" },
          { status: 404 }
...
export async function DELETE(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    await register();
    const deleted = await superCompetition.findByIdAndDelete(id);
    if (!deleted) {
      return NextResponse.json(
        { success: false, error: "Competition not found" },
        { status: 404 }
